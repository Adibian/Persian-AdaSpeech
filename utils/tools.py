...
def to_device(data, device, open_set_speaker=False):
    if len(data) == 12:
        (
            ids,
            speakers,
        ...
        speakers = torch.from_numpy(speakers).long().to(device)  ## speakers are speakers ids
    ...
    if len(data) == 6:
        (ids, speakers, texts, src_lens, max_src_len, mel) = data
        ...
        return (ids, speakers, texts, src_lens, max_src_len, mel)
