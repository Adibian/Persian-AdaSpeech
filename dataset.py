...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speaker_embedding = np.load(speaker_embedding_path).astype(np.float32)
    ...
    def reprocess(self, data, idxs):
        ...
        speaker_embedding = [data[idx]["speaker_embedding"] for idx in idxs]
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speaker_embedding = np.load(speaker_embed_path).astype(np.float32)
        phone = np.array(text_to_sequence(self.text[idx], self.cleaners))
